...
const outputPath = path.join(__dirname, '../customer_quotations.json');
...
async function run(query) {
    try {
        ...
        // Cargar archivo existente para mezclar
        let existingRecords = [];
        ...
        for (let i = 0; i < totalQuotations; i++) {
            ...
            writeProgress({
                ...
                message: `⏳ Descargando ítems de cotización ${qNumber} (${i + 1}/${totalQuotations})...`
            });
        ...
        writeProgress({
            status: "done",
            message: `✅ Proceso completado exitosamente. Se importaron ${newRecords.length} ítems de ${totalQuotations} cotizaciones para el cliente.`
        });
